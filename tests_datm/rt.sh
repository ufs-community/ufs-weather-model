...
# PATHTR - Path to datm-mom6-cice directory
...
while read -r line; do
  ...
  if [[ $line == COMPILE* ]] ; then
      ...
      if [[ $ROCOTO == true ]]; then
        ...
        ./compile.sh $MACHINE_ID "${NEMS_VER}" $COMPILE_NR > ${LOG_DIR}/compile_${COMPILE_NR}.log 2>&1
...
if [[ -e fail_test ]]; then
  ...
  #mv fcst_*.exe modules.* MODULES_AND_EXE
