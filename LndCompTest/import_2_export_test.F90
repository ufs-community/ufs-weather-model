...
contains
  ...
  subroutine import_2_export(State_i, State_e, fldname_i, fldname_e, rc)
    ...
    if (fldchk(State_i,trim(fldname_i))) then
       ...
       if (fldchk(State_e,trim(fldname_e))) then
          write(*,*) 'Field ', trim(fldname_e), ' found'
          call state_getfldptr(State_e,trim(fldname_e), fldptr1d_e, rc=rc)
       endif

       do j=1,size(fldptr1d_i)
          fldptr1d_e(j) = fldptr1d_i(j)
       enddo
    ...
    call ESMF_StateGet(State_e, itemName=trim(fldname_e), field=lfield, rc=rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
    call ESMF_FieldWrite(lfield, fileName='field_lnd_export'//'.nc', &
         timeslice=1, overwrite=.true., rc=rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
    
  end subroutine import_2_export
...
end module import_2_export_test
