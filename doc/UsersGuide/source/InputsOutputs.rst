...
---------------------------------------------
The input files containing grid information and the initial conditions are listed and described in :numref:`Table %s <GridICFiles>`.  The input files for a stand-alone regional configuration including grid information and initial and lateral boundary conditions are listed and described in :numreg:`Table %s <RegionalGridICFiles>`.  Note that the regional grid is referred to as Tile 7 here.
...
Three files control the output that is generated by the UFS Weather Model.  The output files that contain the physical variables are written to NetCDF as shown in Figure X.X. These files may be converted to GRIB2, including remapping and derived variables, by the Unified Post Processor (UPP).  The file names for the physical variables, *fv3_history* and *fv3_history2d*, are specified in the input file *diag_table*. When *quilting* is set to *.true.* for the write components in the input file *model_configure*, the two files of *fv3_history* and *fv3_history2d* are converted into the two NetCDF files named *dynfXXX.nc* and *phyfXXX.nc*. The bases of the file names (*dyn* and *phy*) are specified in the input file ‘model_configure’.
