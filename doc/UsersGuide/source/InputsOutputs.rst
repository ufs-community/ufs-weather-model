...
Two files (*model_configure* and *diag_table*) control the output that is generated by the UFS Weather Model.  The output files that contain the physical variables are written to NetCDF as shown in the figure below.  These files may be converted to GRIB2, including remapping and derived variables, by the Unified Post Processor (UPP).  The file names for the physical variables, *fv3_history* and *fv3_history2d*, are specified in the input file *diag_table*. When *quilting* is set to *.true.* for the write components in the input file *model_configure*, the two files of *fv3_history* and *fv3_history2d* are converted into the two NetCDF files named *dynfXXX.nc* and *phyfXXX.nc*. The bases of the file names (*dyn* and *phy*) are specified in the input file ‘model_configure’.
