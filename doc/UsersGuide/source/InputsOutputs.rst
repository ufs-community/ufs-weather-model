...
------------------------------------
The static input files are listed and described in :numref:`Table %s <FixFiles>`.  Similar files are used for a regional grid but are grid specific and generated by a pre-processing step.
...
.. list-table:: *Input files containing regional grid information and initial and lateral boundary conditions*
   :widths: 35 50 15
   :header-rows: 1
...
:numref:`Section %s <InputNML>`.  The configuration of model output fields is controlled by the combination of diag_table and model_configure, and is described in detail in :numref"`Section %s <Output files>`.
...
Three files control the output that is generated by the UFS Weather Model.  The output files that contain the physical variables are written to NetCDF as shown in |fv3IO| These files may be converted to GRIB2, including remapping and derived variables, by the Unified Post Processor (UPP).  The file names for the physical variables, *fv3_history* and *fv3_history2d*, are specified in the input file *diag_table*. When *quilting* is set to *.true.* for the write components in the input file *model_configure*, the two files of *fv3_history* and *fv3_history2d* are converted into the two NetCDF files named *dynfXXX.nc* and *phyfXXX.nc*. The bases of the file names (*dyn* and *phy*) are specified in the input file ‘model_configure’.

.. |fv3IO| image:: fv3IO.png
