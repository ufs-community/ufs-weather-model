...
------------------------------------
The static input files are listed and described in :numref:`Table %s <FixFiles>`.  Similar files are used for a regional grid but are grid specific and generated by pre-processing utilities.
...
---------------------------------------------
The input files containing grid information and the initial conditions are listed and described in :numref:`Table %s <GridICFiles>`.  The input files for a stand-alone regional configuration including grid information and initial and lateral boundary conditions are listed and described in :numref:`Table %s <RegionalGridICFiles>`.  Note that the regional grid is referred to as Tile 7 here.
...
.. list-table:: *Regional input files containing grid information and initial and lateral boundary conditions*
   :widths: 35 50 15
   :header-rows: 1
   ...
   * - Filename
     ...
     - Date-dependent
   * - Cxx_grid.tile7.nc
     - Cxx grid information for tile 7
     -
   * - gfs_ctrl.nc
     - NCEP NGGPS tracers, ak, and bk
     - ✔
...
:numref:`Section %s <InputNML>`.  The run-time configuration of model output fields is controlled by the combination of diag_table and model_configure, and is described in detail in :numref:"`Section %s <Output files>`.
...
Note that the sfcf* and atmf* files are not output on the 6 tiles, but instead as a single global gaussian grid file.  The specifications of the output files (format, projection, etc) may be overridden in the *model_configure* input file.

Three files control the output that is generated by the UFS Weather Model.  The output files that contain the physical variables are written to NetCDF as shown in :numref:`Figure %s <fv3IO>` These files may be converted to GRIB2, including remapping and derived variables, by the Unified Post Processor (UPP).  The file names for the physical variables, *fv3_history* and *fv3_history2d*, are specified in the input file *diag_table*. When *quilting* is set to *.true.* for the write components in the input file *model_configure*, the two files of *fv3_history* and *fv3_history2d* are converted into the two NetCDF files named *dynfXXX.nc* and *phyfXXX.nc*. The bases of the file names (*dyn* and *phy*) are specified in the input file ‘model_configure’.
