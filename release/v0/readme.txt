...
3. Run a default experiment
   The default setting is to run an experimen  with non-hydrostatic 32-bit fv3 dycore
   (executable fv3_gfs_nh.prod.32bit.x with C96 resoultion, ouput are on 6 tile cubed-sphere grid, 
...
A2: Initial conditions from WCOSS Surge are in a fixed location on WCOSS Surge/Luna, Theia and jet
   ...
   The iniitial conditions contain cold start initial conditions for the following three cases. 
...
A4: First view the job card to see how to submit forecast batch jobs on WCOSS Cray.
    ...
    b) run different case:
      ...
      change defaul CDATE from 2016092900 to 2016011812 or 2016081200


    c) Run different executable:
       export MODE=32bit                 # dycore precision:   32bit, 64bit
       export TYPE=nh                    # hydrostatic option: nh, hydro
       change MODE to 64bit or TYPE to hydro, you will get different forecast exectuable, please see section 2a.
    ...
    f) run with different nodes
       export NODES=16                   # number of nodes for forecst job
...
Q6. What other external libaries are used?
